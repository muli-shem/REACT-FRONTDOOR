...
const SetPasswordPage = () => {
  ...
  useEffect(() => {
    if (!email) {
      toast.error('Invalid access. Email parameter missing.');
      navigate('/login');
    }
  }, [email, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/members/set-password/', {
        email,
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-primary mb-2">Set Your Password</h1>
          <p className="text-gray-600">Welcome to G-NET! Create a secure password to access your account.</p>
          {email && (
